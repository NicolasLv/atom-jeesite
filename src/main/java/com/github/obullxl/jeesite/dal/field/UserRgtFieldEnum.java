...
/**
 * A fields enum object database table <tt>atom_user_rgt</tt>.
 */
public enum UserRgtFieldEnum implements EnumBase {
	//
    ID(0, "id", "id"),
	//
    NAME(1, "name", "name"),
	//
    RGT_CODE(2, "rgt_code", "rgt_code"),
	//
     ...
     * CTOR
     */
    private UserRgtFieldEnum(int index, String field, String remark) {
        this.index = index;
        this.field = field;
		this.remark = remark;
    }
    ...
    /**
     * 根据字段名获取枚举内�?	 */
    public static UserRgtFieldEnum findByField(String field) {
        field = StringUtils.lowerCase(field);
        if(StringUtils.isBlank(field)) {
            return null;
        }
        
        for(UserRgtFieldEnum enm : values()) {
